...
# encoding the entire text dataset and store in torch.Tensor
data = torch.tensor(encode(text), dtype=torch.long)

# Let's now split up the data into train and val sets
n = int(0.9 * len(text))
train_data = data[:n]
val_data = data[n:]
...
class Block(nn.Module):
    ...
    def __init__(self, n_embed, n_head):
        # n_embed: embedding dim, n_head: the number of heads we'd like
        super().__init__()
